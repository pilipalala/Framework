...
public class GoodsAdapter extends BaseAdapter implements StickyListHeadersAdapter {
    ...
    public GoodsAdapter(ArrayList<GoodsItem> dataList, Context context) {
        this.dataList = dataList;
    ...
    class ItemViewHolder implements View.OnClickListener {
        ...
        public void bindData(GoodsItem item) {
            ...
            if (mContext == null) {
                item.count = ShopCartFragment.getSelectedItemCountById(item.id);
            } else {
        ...
        public void onClick(View v) {
            ...
            switch (v.getId()) {
                case R.id.tvAdd: {
                    if (mContext == null) {
                        countt = ShopCartFragment.getSelectedItemCountById(item.id);
                    } else {
                        countt = activity.getSelectedItemCountById(item.id);
                    }
                    if (countt < 1) {
                    ...
                    if (mContext == null) {
                        ShopCartFragment.add(item, false);
                    } else {
                        activity.add(item, false);
                    }

                    countt++;
                    tvCount.setText(String.valueOf(countt));
                    int[] loc = new int[2];
                    v.getLocationInWindow(loc);
                    if (mContext == null) {
                        ShopCartFragment.playAnimation(loc);
                    } else {
                ...
                case R.id.tvMinus: {
                    int count;
                    if (mContext == null) {
                        count = ShopCartFragment.getSelectedItemCountById(item.id);
                    } else {
                        count = activity.getSelectedItemCountById(item.id);
                    }
                    if (count < 2) {
                    ...
                    count--;
                    if (mContext == null) {
                        ShopCartFragment.remove(item, false);
                    } else {
